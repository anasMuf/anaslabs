...
| State Management | Jotai (jika diperlukan global state) |
| API Gateway | Golang (Gin / Chi / Huma) |
...
| Database | PostgreSQL + Prisma (atau GORM untuk Golang) |
| Validation | Zod (frontend) + Go validator (backend) |
...
│   │   ├── src/
│   │   │   ├── features/
│   │   │   │   ├── analyzer/   # Input & result UI
│   │   │   │   ├── history/    # Riwayat analisis
│   │   │   │   └── maqam/      # Maqam info pages
│   │   │   └── routes/
...
│       └── requirements.txt
├── packages/
│   ├── shared-types/           # TypeScript types (generated dari OpenAPI)
│   └── maqam-data/             # JSON template semua maqam
├── docker-compose.yml
└── pnpm-workspace.yaml
```
